...
BASELINE_METADATA_PATH = ROOT / "data" / "processed" / "baseline_metadata.json"
...
def model_metric_rows(results: dict, baseline_p50: float) -> list[list[str]]:
    rows = [["Model", "ROC AUC", "Avg precision", "Precision@50", "Recall", "F1"]]
    for name, metrics in results["models"].items():
        rows.append(
            [
                name,
                pct_metric(metrics.get("roc_auc", 0)),
                pct_metric(metrics.get("average_precision", 0)),
                pct_metric(metrics.get("precision_at_50", 0)),
                pct_metric(metrics.get("recall", 0)),
                pct_metric(metrics.get("f1", 0)),
            ]
        )
    # Add baseline row
    rows.append(
...
def footer(canvas, doc) -> None:
    ...
    canvas.drawString(
        LEFT_MARGIN,
        0.22 * inch,
        "FlyRank ML Internship 2026 — Content Underperformance Predictor — anonymized data",
    )
...
def build_pdf() -> None:
    ...
    # Load baseline metadata for P@50
    baseline_p50 = 0.0
    if BASELINE_METADATA_PATH.exists():
        baseline_meta = load_json(BASELINE_METADATA_PATH)
        baseline_p50 = baseline_meta.get("precision_at_50", 0.0)
        print(f"Loaded baseline P@50: {baseline_p50:.4f}")
    else:
        print(f"Warning: {BASELINE_METADATA_PATH} not found, baseline P@50 = 0")
    ...
    # Compute base rate from queue
    base_rate = 0.0
    if queue_rows:
        declining = sum(1 for r in queue_rows if r.get("is_below_peer_median", "0") == "1")
        base_rate = declining / len(queue_rows)
    ...
    document = SimpleDocTemplate(
        ...
        title="FlyRank Content Underperformance Predictor — Results",
        author="FlyRank",
    )

    # Build P@50 comparison with baseline
    p50_comparison = [
        (name, metrics["precision_at_50"])
        for name, metrics in results["models"].items()
    ]
    p50_comparison.append(("Baseline Rules", baseline_p50))

    story = [
        Paragraph("FlyRank ML Internship 2026", styles["title"]),
        Paragraph("Content Underperformance Predictor — Refresh Opportunity Results PDF", styles["subtitle"]),
        card_grid(
            [
                (format_int(summary["rows_scored"]), "Rows scored", "Anonymized BigQuery export"),
                (best_model, "Best model", "Selected by Precision@50"),
                (pct_metric(best_metrics["roc_auc"]), "ROC AUC", "Ranking separation quality"),
                (pct_metric(best_metrics["precision_at_50"]), "Precision@50", "Top-50 underperformance hit rate"),
                (format_int(summary["high_confidence_rows"]), "High confidence", "Rows ready for manual review"),
                (pct_metric(base_rate), "Base rate (declining)", "Below-peer-median CTR prevalence"),
                (format_score(summary["top_queue_score"]), "Top queue score", "0–100 final priority score"),
                ("client holdout", "Validation style", "GroupShuffleSplit by client_hash_id"),
            ],
            styles,
        ),
        Spacer(1, 0.16 * inch),
        ...
        styled_table(
            [
                ...
                [
                    ...
                    "A ranked queue of content pages predicted to have below-median CTR relative to their position bucket.",
                ],
                [
                    "What is the model doing?",
                    "It predicts underperformance risk, then blends model probability with a transparent rule baseline for final prioritization.",
                ],
                [
                    "What should mentors inspect first?",
                    "High-confidence rows with `refresh`, `refresh_and_review_ctr`, or `expand_and_refresh` actions.",
                ],
                [
        ...
        Paragraph(
            ...
            "All claims are directional and measured, not causal.",
            styles["body"],
        ),
        PageBreak(),
        Paragraph("1. Model Quality", styles["section"]),
        styled_table(
            model_metric_rows(results, baseline_p50),
            col_widths=[2.0 * inch] + [1.25 * inch] * 5,
            right_align_from=1,
        ),
        Spacer(1, 0.18 * inch),
        HorizontalBarChart(
            "Precision@50 comparison (model vs baseline)",
            p50_comparison,
            value_formatter=lambda value: f"{value:.3f}",
            color_palette=[BRAND_BLUE, BRAND_TEAL, BRAND_PURPLE, BRAND_ORANGE, BRAND_RED],
        ),
        Spacer(1, 0.1 * inch),
        styled_table(
            [
                ["Metric", "Plain-English interpretation"],
                ["ROC AUC", "How well the model ranks underperforming pages ahead of normal pages."],
                ["Average precision", "Quality of the ranked list when positives are mixed with negatives."],
                ["Precision@50", "How many of the top 50 recommended pages are actually below peer-median CTR."],
                ["Recall", "How many underperforming pages are captured at the default 0.5 probability threshold."],
                ["Baseline Rules", "Transparent rule-based score: old + never optimized + thin + no keyword."],
            ],
        ...
        HorizontalBarChart(
            ...
            [
                ...
                for item in results["best_model"]["top_features"][:12]
            ],
            value_formatter=lambda value: f"{value:.3f}",
        ...
        Spacer(1, 0.1 * inch),
        Paragraph(
            "Strong signals are mostly freshness and optimization related: "
            "optimized_and_fresh, breadth_vs_depth, log_age, visibility flags, and content depth. "
            "The model prioritizes pages that are old, thin, or never optimized.",
            styles["body"],
        ),
        PageBreak(),
        ...
        Paragraph(
            "These rows are anonymized. Use `content_hash_id` only to join back internally when mentors are ready to inspect real pages.",
            styles["small"],
        ),
        styled_table(
            [
            ...
            + [
                [
                    ...
                    format_int(row["total_impressions"]),
                    format_int(row["ga4_sessions"]),
                    "Yes" if row.get("is_below_peer_median") == "1" else "No",
